...
const int ACTION_LOWER = 0;
const int ACTION_SOLIDIFY = 0;
...
void main() {
	...
	if (action == ACTION_CREATE) {
		...
		float next_dirt;
		if (min_i != 0 && min_j != 0){
			if ((level - min_level) / DZ > 1) {
				float diff = (level - min_level) - DZ;
				float extra = max((diff - DZ)/2, dirt);
			
				dirt -= extra;
				next_dirt = extra;
			}
		} else if (min_i != 0 || min_j != 0){
			if ((level - min_level) > DX) {
				float extra = (level - min_level) - DX;
				dirt -= DX;
				next_dirt = DX;
			}
		}
